...
export async function GET(req: NextRequest, { params }: { params: { referenceId: string } }) {
  const landlord = await getCurrentLandlord(req);
  if (!landlord) return unauthorizedResponse();

  const payment = await prisma.payment.findFirst({
    where: { reference: params.referenceId, tenant: { property: { landlordId: landlord.id } } },
  });
  if (!payment) {
    return Response.json({ error: "Paiement introuvable" }, { status: 404 });
  }

  const result = await checkRequestToPayStatus(params.referenceId);
